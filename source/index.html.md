...
## 1.2.3 2020年1月21日 【新增：批量获取聚合行情接口。 2-14 修改接口，新增字段】
...
### 4、修改获取订单明细信息接口（返回参数中的 data 和 trades 下各新增以下字段:real_profit(真实收益)。同时 trades 下新增每笔成交收益字段：profit（收益））
 - 接口名称：获取订单明细信息
 - 接口类型：私有接口
...
### 10、修改订阅订单成交数据接口（返参外层新增real_profit字段，表示真实收益，类型decimal）
 - 接口名称：订阅订单成交数据
 - 接口类型：私有接口
 - 订阅地址：orders.$symbol
