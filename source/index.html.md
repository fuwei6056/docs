...
## 1.1.4 2020看1月27日 【新增：批量获取聚合行情接口、获取标记价格的 K 线数据、订阅标记价格 K 线数据 WS 接口、请求标记价格 K 线数据 WS 接口。5-17 修改接口，新增字段】
...
### 7、修改获取订单明细信息接口（返回参数中的 data 和 trades 下各新增以下字段:real_profit(真实收益)。同时 trades 下新增每笔成交收益字段：profit（收益））
 - 接口名称：获取订单明细信息
 - 接口类型：私有接口
...
### 13、修改订阅订单成交数据接口（返参外层新增real_profit字段，表示真实收益，类型decimal）
 - 接口名称：订阅订单成交数据
 - 接口类型：共公接口
...
读取     |  市场行情接口           |  /index/market/history/swap_mark_price_ kline  |                 GET        |  获取标记价格的 K 线数据                 |  否  |
...
| 读取    |  市场行情接口 | market.$contract_code.mark_price.$period                    | sub  | 订阅标记价格 K 线数据              |       否      |
| 读取    |  市场行情接口 | market.$contract_code.mark_price.$period                    | req  | 请求标记价格 K 线数据              |       否      |
...
| profit                  | decimal | 收益（使用持仓均价计算，不包含仓位跨结算的已实现盈亏。）             |
...
| real_profit | decimal | 真实收益（使用开仓均价计算，包含仓位跨结算的已实现盈亏。）  |  |
